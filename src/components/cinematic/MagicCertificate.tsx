...
export const MagicCertificate: React.FC<MagicCertificateProps> = ({ onSubmitName, onAgree, onContinue, initialStep = 'trap', initialName = '' }) => {
    ...
    const certDeclarePrefix = t('onboarding.certDeclarePrefix');
    const certDeclareSuffix = t('onboarding.certDeclareSuffix');
    const firstLine = certDeclarePrefix + name + certDeclareSuffix;
    ...
    useEffect(() => {
        if (step !== 'certificate' || declarationTyped < DECLARATION_FULL.length) return;
        const to = setTimeout(() => {
            if (mountedRef.current) setShowSeal(true);
        }, 1200);
        return () => clearTimeout(to);
    }, [step, declarationTyped, DECLARATION_FULL.length]);
    ...
    return (
        ...
        >
            <AnimatePresence mode="wait">
                ...
                {/* SCENE 2: THE CERTIFICATE — same design as CertificateApp (trading bg, gold frame, gold-foil, seal) */}
                {step === 'certificate' && (
                    <motion.div
                        key="certificate"
                        className="absolute inset-0 min-h-screen bg-[#060B14] flex flex-col items-center justify-center p-4 md:p-8 overflow-auto font-sans"
                        initial={{ scale: 0.98, opacity: 0, filter: 'blur(6px)' }}
                        animate={{ scale: 1, opacity: 1, filter: 'blur(0px)' }}
                        exit={{ scale: 0.98, opacity: 0, filter: 'blur(8px)', transition: { duration: 1 } }}
                    ...
                    >
                        <style>{`
                            ...
                            .mc-font-serif { font-family: 'Playfair Display', serif; }
                            .mc-font-signature { font-family: 'Great Vibes', cursive; }
                            .mc-font-cinzel { font-family: 'Cinzel', serif; }
                            .gold-foil-text {
                        ...
                        <div className="absolute inset-0 pointer-events-none z-0">
                            ...
                            {candles.map((candle) => (
                                <motion.div
                                    key={candle.id}
                                    className="absolute"
                                    style={{ left: candle.left, top: 0, bottom: 0, width: '6px', opacity: candle.opacity }}
                                    initial={{ y: 10 }}
                                    animate={{ y: -10 }}
                                    transition={{ duration: 3, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut', delay: candle.delay }}
                                >
                                    <div className={`absolute left-1/2 -translate-x-1/2 w-[1px] ${candle.isBull ? 'bg-emerald-500' : 'bg-rose-500'}`} style={{ top: candle.wickTop, height: candle.wickHeight }} />
                                    <div className={`absolute left-0 w-full rounded-[1px] ${candle.isBull ? 'bg-emerald-500' : 'bg-rose-500'}`} style={{ top: candle.bodyTop, height: candle.bodyHeight }} />
                                </motion.div>
                            ))}
                        </div>
                        ...
                        >
                            ...
                            <div className="relative z-10 mc-font-serif flex flex-col items-center pt-2">
                                <motion.div
                                    initial={{ opacity: 0, y: -20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                ...
                                >
                                    ...
                                    <h2 className="mc-font-cinzel text-amber-500 uppercase tracking-[0.5em] text-sm md:text-lg font-bold mb-4 drop-shadow-[0_0_12px_rgba(245,158,11,0.6)]">
                                ...
                                <div className="w-full mt-10 flex flex-col md:flex-row justify-between items-stretch md:items-end gap-8 relative min-h-[140px]">
                                    <AnimatePresence>
                                        {showSeal && (
                                            ...
                                            >
                                                <p className="mc-font-signature text-[54px] leading-none text-amber-50/90 mb-3 drop-shadow-[0_4px_6px_rgba(0,0,0,0.9)] relative z-10 transform -rotate-3">
                                                    {name}
                                                </p>
                                                <div className="w-full h-[1.5px] bg-gradient-to-r from-amber-800 via-amber-600/50 to-transparent mb-3" />
                                                <p className="text-[11px] md:text-[13px] tracking-[0.25em] text-amber-500/70 uppercase mc-font-cinzel font-bold">
